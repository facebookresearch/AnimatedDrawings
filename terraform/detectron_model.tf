...
resource "aws_ecs_service" "detectron_ecs_service" {
  name        = "${var.detectron_service_name}-QA-UPDATE"
  launch_type = "FARGATE"
...
resource "aws_appautoscaling_target" "detect_target" {
  max_capacity = 10
  min_capacity = 5
  resource_id = "service/${aws_ecs_cluster.ml_devops_cluster.name}/${aws_ecs_service.detectron_ecs_service.name}"
...
resource "aws_appautoscaling_policy" "requests" {
  ...
  target_tracking_scaling_policy_configuration {
    ...
    predefined_metric_specification {
      ...
      resource_label         = "${aws_lb.ml_devops_alb.arn_suffix}/${aws_alb_target_group.detectron_tg.arn_suffix}"
    }
  }
}
