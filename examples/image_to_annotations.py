...
def image_to_annotations(img_fn: str, out_dir: str) -> None:
    ...
    resp = requests.post("http://dev-60yfmd-alb-647026402.ap-northeast-1.elb.amazonaws.com/predictions/drawn_humanoid_detector", files=request_data, verify=False)
    ...
    resp = requests.post("http://dev-60yfmd-alb-647026402.ap-northeast-1.elb.amazonaws.com/predictions/drawn_humanoid_pose_estimator", files=data_file, verify=False)
    if resp is None or resp.status_code >= 300:
