...
def combine_cfg_and_args():
    ...
    main_config_file = osp.join(script_source_path, r'offline_lib/mvc/video_common_base.yaml')
    ...
    def get_args():
        ...
        parser.add_argument('--src_character_config', type=str, default='characters/char6/char_cfg.yaml',
                            help='The source estimated keypoints config file.')
        parser.add_argument('--src_retarget_config', type=str, default='config/retarget/four_legs.yaml',
                            help='The source sketch image path.')
        parser.add_argument('--src_motion_config', type=str, default='config/motion/zombie.yaml',
                            help='Source motion config information for corresponding motion file.')
    ...
    cfg.scene.ANIMATED_CHARACTERS[0].character_cfg = args.src_character_config
    # if use video, then automatacally use h36m motion config file
    # motion_file_suffix = osp.basename(args.src_motion_config).split('.')[-1]
    # if motion_file_suffix not in VIDEO_SUFFIX_LIST:
    #     cfg.scene.ANIMATED_CHARACTERS[0].motion_cfg = args.src_motion_config
    cfg.scene.ANIMATED_CHARACTERS[0].motion_cfg = args.src_motion_config
    cfg.scene.ANIMATED_CHARACTERS[0].retarget_cfg = args.src_retarget_config
...
def sketch_detector_and_estimator(main_cfg, sketch_cfg, img_path: str):
    print('>>> In common mode, sketch detector and estimation should manual upload!!')
...
def regenerate_config(main_cfg, character_cfg, motion_cfg, retarget_cfg):
    def remove_old_and_rename_new(src_path: str, target_path: str):
    ...
    if osp.exists(osp.join(osp.dirname(texture_image_path), f'texture.{img_suffix}')):
        os.remove(osp.join(osp.dirname(texture_image_path), f'texture.{img_suffix}'))
    os.rename(texture_image_path, osp.join(osp.dirname(texture_image_path), f'texture.{img_suffix}'))

    if osp.exists(osp.join(osp.dirname(mask_image_path), f'mask.{img_suffix}')):
        os.remove(osp.join(osp.dirname(mask_image_path), f'mask.{img_suffix}'))
    os.rename(mask_image_path, osp.join(osp.dirname(mask_image_path), f'mask.{img_suffix}'))

    if not osp.exists(osp.join(target_character_cfg_dir, 'char_cfg.yaml')):
