...
def threshold(img): 
    " Convert to grayscale, adaptive threshold, invert"
...
def morph_ops(im_in):
    ...
    im_inter = cv2.morphologyEx(im_in, cv2.MORPH_CLOSE, kernel, iterations=2)
    im_out = cv2.morphologyEx(im_inter, cv2.MORPH_DILATE, kernel, iterations=2)
    return im_out
