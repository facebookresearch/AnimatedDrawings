...
Most of the available parameters are defined in `animated_drawings/mvc_base_cfg.yaml`. This file should **not** be changed. Instead, create a new mvc config file containg *only* the parameters that need to be modified.
...
See `mvc/interactive_window.yaml`, `mvc/headless_video_export.yaml`, or `mvc/window_video_export.yaml` for examples.
