...
class Transform():
    ...
    def update_transforms(self, parent_dirty_bit: bool = False, recurse_on_children: bool = True,
                          update_ancestors: bool = False) -> None:
        """
    ...
    def reset_offset(self, pos: Union[npt.NDArray[np.float32], Vectors]) -> None:
        '''
        直接重置整个offset，不在之前的offset上做累加
        :param pos:
        :return:
        '''
        if isinstance(pos, Vectors):
    ...
    def get_init_local_offset(self) -> npt.NDArray[np.float32]:
        '''
        该函数仅在最初初始化节点时为了获取初始的offset时使用
        :return: (x,y,z), 以 np.array包裹
        '''
    ...
    def get_right_up_fwd_vectors(self) -> Tuple[
