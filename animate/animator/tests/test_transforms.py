...
def test_init():
    t = Transform()
    for m in [t.rotate_m, t.translate_m, t.scale_m, t.local_transform, t.world_transform]:
...
def test_set_position():
    ...
    assert np.array_equal(
        t.local_transform[:-1, -1], np.array([2.0, 2.0, 2.0]))
...
def test_offset():
    ...
    t.update_transforms()
    assert np.array_equal(
        t.local_transform[:-1, -1], np.array([3.0, 3.0, 3.0]))
...
def test_update_transforms():
    ...
    assert np.array_equal(
        t2.world_transform[:-1, -1], np.array([3.0, 0.0, 0.0]))
...
def test_rotate():
    ...
    assert np.isclose(t.local_transform, m).all()
